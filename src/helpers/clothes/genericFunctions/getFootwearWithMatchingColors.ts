 ...
 */
export const getFootwearWithMatchingColors = (
  products: ClothesType[],
  allowedColors: ColorNameType[]
): ClothesType[] =>
  products.flatMap((product) => {
    if (!product.colors.length) return [];

    const matchedColors = allowedColors.flatMap((item) =>
      filterColorsByAllowedList(product.colors, item)
    );
